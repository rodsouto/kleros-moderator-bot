...
import {addBan, getBot, getGuildUserID, getGuildURL, getRules, isMod} from "../../db";
...
const callback: CommandCallback = async (bot: TelegramBot, msg: TelegramBot.Message) => {
    ...
    const guildBotUserID = await getGuildUserID(msg.chat.id, 'telegram');
    ...
    if (guildBotUserID === msg.reply_to_message.from.id) {
        await bot.sendMessage(msg.chat.id, `The Guild Bot can't be banned.`);
        return;
    }
    ...
    try {
        ...
        const guildBotURL = await getGuildURL(msg.chat.id, 'telegram');
        ...
        const {questionId, questionUrl: appealUrl} = await banUser(hasBanningPermission, fromUsername, guildBotURL, rules, privateKey);
        ...
        if (hasBanningPermission) {
            ...
            await bot.sendMessage(msg.chat.id, `To add evidence to the case, reply to a message with ther addevidence command followed by the questionID ${questionId} and an explanation.`, {parse_mode: 'Markdown'});
